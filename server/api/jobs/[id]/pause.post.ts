...
import { pauseJob } from '../../../storage/jobs'
...
export default defineEventHandler((event) => {
  ...
  if (!session || !jobId.success) throw createError({ statusCode: 400, statusMessage: 'Запрос паузы недействителен.' })
  ...
  if (!pauseJob(database, user.id, jobId.data)) {
    throw createError({ statusCode: 409, statusMessage: 'Это задание нельзя приостановить.' })
  }
